...
def list_available_tests() -> None:
    ...
    tests = {
        ...
        "readcct": "Read CCT range parameters from CCT devices",
        "groups": "Discover DALI groups",
        "readgroup": "Read group details with device list",
        "groupcontrol": "Test group turn_on/turn_off with devStatus verification",
        "groupbrightness": "Test group brightness across multiple levels (100%/75%/50%/25%/10%/min)",
        ...
        "creategroup": "Create a test group (cumulative, not auto-deleted)",
        "createscene": "Create a test scene (cumulative, not auto-deleted)",
        "cleanup": "Delete all test groups and scenes (name starts with 'test_')",
        # "identifygateway": "Identify gateway (makes gateway LED blink)",
        "identifydevice": "Identify devices (makes device LEDs blink)",
        "busscan": "Bus scan - physical DALI bus device scan (vs cached discovery)",
        "busscanbusy": "Bus scan after device control (test bus busy handling)",
        "stopscan": "Stop an in-progress bus scan",
