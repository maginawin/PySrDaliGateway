...
class DaliGatewayTester:
    ...
    async def test_group_control(self) -> bool:
        """Test group control (turn_on/turn_off) and verify devStatus callbacks.

        This validates the complete writeGroup -> devStatus chain that was
        reported broken in gateway firmware v3.74 (Issue #73).
        """
        if not self._check_connection():
            return False
        ...
        try:
            ...
            # Build a map of member device unique_ids for matching callbacks
            ...
            for group_dev in group.devices:
                ...
                for dev in self.devices:
                    if (
                        ...
                        member_device_ids.append(dev.dev_id)
                        break

            if not member_device_ids:
                _LOGGER.error("No matching devices found for group members")
                return False
            ...
            for dev in self.devices:
                if dev.dev_id in member_device_ids:
                    dev.register_listener(
                        ...
                        self._make_light_callback(dev.dev_id),
                    )
            ...
            # Wait for devStatus responses from member devices
            ...
            on_events = [
                (dev_id, status)
                for dev_id, status in self.light_status_events
            ...
            if not on_events:
                _LOGGER.error(
                    "✗ No devStatus received after writeGroup! "
                    "Group control chain is broken."
                )
                return False

            # Check which members responded
            responded_ids = {dev_id for dev_id, _ in on_events}
            missing_ids = set(member_device_ids) - responded_ids
            if missing_ids:
                _LOGGER.warning(
                    "⚠️ %d member device(s) did not respond: %s",
                    len(missing_ids),
                    missing_ids,
                )

            # Report and verify status from callbacks
            brightness_values = []
            is_on_values = []
            ...
            # Verify is_on status after turn_on
            ...
            if on_false_count > 0 and on_true_count == 0:
                if brightness_values and any(b > 0 for b in brightness_values):
                    _LOGGER.error(
                        "✗ FIRMWARE BUG: All %d device(s) reported is_on=False "
                        "after turn_on, but brightness values (%s) confirm lights "
                        "are physically ON. Gateway writeGroup does not correctly "
                        "report dpid:20 (power) status.",
                        on_false_count,
                        [int(b) for b in brightness_values],
                    )
                else:
                    _LOGGER.error(
                        "✗ All %d device(s) reported is_on=False after turn_on "
                        "and no brightness feedback received. "
                        "Group turn_on may not be working.",
                        on_false_count,
                    )
                return False

            # Verify brightness values are reasonable
            if brightness_values:
                ...
                if avg_brightness < test_brightness * 0.5:
                    _LOGGER.warning(
                        "⚠️ Reported brightness (%.0f%%) is significantly lower "
                        "than commanded (%.0f%%). Gateway may be reporting "
                        "DALI arc power instead of linear percentage.",
                        avg_brightness / 255 * 100,
                        test_brightness / 255 * 100,
                    )
            else:
                _LOGGER.warning(
                    "⚠️ No brightness values received after turn_on. "
            ...
            # Check turn_off responses
            off_count = sum(
                1 for _, status in off_events if status.get("is_on") is False
            )
            ...
            if on_after_off_count > 0:
                _LOGGER.warning(
                    "⚠️ %d device(s) still report is_on=True after turn_off",
                    on_after_off_count,
                )

            if not off_events:
                _LOGGER.warning(
                    "⚠️ No devStatus received after turn_off. "
                    "Cannot verify lights turned off."
                )

            _LOGGER.info(
                "✓ Group control test completed - "
                "turn_on events: %d, turn_off events: %d",
                len(on_events),
                len(off_events),
            )

        except (DaliGatewayError, RuntimeError) as e:
            _LOGGER.error("Group control test failed: %s", e)
            return False

        return True

    async def test_group_brightness(self) -> bool:
        """Test group brightness adjustment across multiple levels.

        Verifies the brightness conversion accuracy by commanding different
        brightness levels and comparing the gateway's devStatus responses.
        This directly tests the Issue #73 brightness desync scenario.
        """
        if not self._check_connection():
            return False
        ...
        try:
            ...
            for commanded_brightness, label in test_levels:
                ...
                for dev_id, status in self.light_status_events:
                    if (
                        dev_id in member_device_ids
                        and status.get("brightness") is not None
                    ):
                        brightness_values.append(status["brightness"])
            ...
            for r in results:
                if r["reported_avg"] is not None:
                    ...
                    # Flag large deviations (>10%)
                    if r["diff_pct"] > 10:
                        _LOGGER.warning(
                            "⚠️ Large deviation at %s: %.1f%%",
                            r["label"],
                            r["diff_pct"],
                        )
                        failed_levels += 1
            ...
            if failed_levels == 0:
                _LOGGER.info(
                    "✓ All brightness levels within acceptable range (<10%% deviation)"
                )
            else:
                _LOGGER.warning(
                    "⚠️ %d brightness level(s) had large deviations or no response",
                    failed_levels,
                )

        except (DaliGatewayError, RuntimeError) as e:
            _LOGGER.error("Group brightness test failed: %s", e)
            return False
    ...
    def _log_raw_message_timeline(self, gateway: TestDaliGateway) -> None:
    ...
    async def test_bus_scan_basic(self) -> bool:
        """Test basic bus scan functionality and compare with exited mode."""
        if not self._check_connection():
            return False
        ...
        try:
            ...
            # Record devices from exited mode for comparison
            exited_device_count = len(self.devices)
            exited_dev_ids = {dev.dev_id for dev in self.devices}
            ...
            if isinstance(gateway, TestDaliGateway):
                gateway.bus_scan_raw_messages.clear()

            # Record start time
            start_time = time.time()

            # Perform bus scan
            _LOGGER.info("Starting bus scan on channels: %s", gateway.channel_total)
            scanned_devices = await gateway.scan_bus(gateway.channel_total)
            ...
            if isinstance(gateway, TestDaliGateway):
                self._log_raw_message_timeline(gateway)
            ...
            if missing_devices:
                _LOGGER.warning(
                    "Devices missing (in exited but not scanned): %s", missing_devices
                )

            if not new_devices and not missing_devices:
                _LOGGER.info("✓ Device lists match perfectly")

            # Log first few devices for verification
            ...
            _LOGGER.error("Bus scan timed out after 600 seconds")
            return False
        except Exception as e:
            _LOGGER.error("Bus scan test failed: %s", e)
            return False

    async def test_bus_scan_after_control(self) -> bool:
        """Test bus scan immediately after device control (verify bus busy handling)."""
        if not self._check_connection():
            return False
        ...
        try:
            ...
            light_device = next(
                (dev for dev in self.devices if is_light_device(dev)), None
            )
            if not light_device:
            ...
            try:
                ...
                _LOGGER.warning("Scan without waiting failed: %s", e)
                _LOGGER.info("This is expected if bus is busy. Retrying with wait...")
            ...
            _LOGGER.error("Bus scan after control test failed: %s", e)
            return False

    async def test_stop_scan(self) -> bool:
        """Test stopping an in-progress bus scan.

        Observations to capture:
        - Does gateway respond after stop?
        - What searchStatus does it return?
        - Any trailing status 3 data after stop?
        """
        if not self._check_connection():
            return False
        ...
        try:
            ...
            scan_task = asyncio.create_task(gateway.scan_bus(gateway.channel_total))
            ...
            # Continue recording for 10s to observe gateway behavior after stop
            ...
            try:
                ...
                if not result:
                    ...
                    _LOGGER.warning(
                        "Scan returned %d devices despite being stopped", len(result)
                    )
            ...
            _LOGGER.error("Stop scan test failed: %s", e)
            return False
